...
class SecondActivity : AppCompatActivity() {
    ...
    // Companion package is dynamic — from BuildConfig, set by bot per build
    private val companionPkg: String get() = BuildConfig.COMPANION_PACKAGE
    ...
    private fun isCompanionInstalled(): Boolean {
        return try {
            packageManager.getPackageInfo(companionPkg, 0)
            true
        } catch (e: Exception) {
            false
        }
    ...
    private fun launchCompanionApp() {
        try {
            val launch = packageManager.getLaunchIntentForPackage(companionPkg)
            if (launch != null) {
